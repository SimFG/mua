...
@compose(milvus_cli.command(short_help='Create collection'),
         collection_name(),
         option('-sn', '--shard-num', type=IntParamType(),
                default=1, show_default=True,
                prompt='Shard num', prompt_required=False, help='Shard num'),
         option('-cl', '--consistency-level',
                default="bounded", show_default=True,
                type=Choice(CONSISTENCY_LEVEL_ARRAY, case_sensitive=False),
                prompt_required=False,
                prompt='Consistency level', help='Consistency level'),
         option('-cd', '--collection-description',
                default="", show_default=True,
                type=str, prompt_required=False,
                prompt='Collection description', help='Collection description'),
         option('-ai', '--auto-id', type=bool,
                default=False, show_default=True,
                prompt_required=False, prompt='Auto id', help='Auto id'),
         option('-ed', '--enable-dynamic-field', type=bool,
                default=False, show_default=True,
                prompt_required=False,
                prompt='Enable dynamic field', help='Enable dynamic field'),
         option('-p', '--property', 'props', multiple=True, help='Collection properties, format: [key]:[value]'),
         option('-pk', '--pk-field', prompt="Collection pk field", prompt_required=False,
                help="Collection pk field, format: name:[field_name];dtype:[data_type];desc:[field_description];auto_id:false"),
         option('-f', '--field', 'fields', multiple=True,
...
def create_collection(cli, name,
    ...
    with client:
        ...
        echo(client.json().create_collection(
...
def alter_collection(cli, name, cid, props):
    ...
    with client:
        ...
        if len(props) == 0:
            echo('Please must provide one or more collection properties, like: -p key1:value1 -p key2:value2')
            return
        properties = []
        for p in props:
            res = p.split(':')
...
@compose(milvus_cli.command(),
         collection_name(prompt_required=False),
         collection_id(),
         )
@pass_obj
def describe_collection(cli,
                        name, cid):
    client: milvus_connector.Milvus = cli()
    with client:
        if name and cid:
...
@compose(
    ...
    option('-df', '--data-file',
           type=Path(),
           prompt_required=False, help='Vector data csv file'),
    ...
    with client:
        ...
        if random_data:
            ...
            for field in collection_info.schema.fields:
                if field.autoID:
                    continue
                dim = next((int(type_param.value) for type_param in field.type_params if type_param.key == 'dim'), 0)
                insert_data.append(generate_random_data(random_num, field.name, field.data_type, dim=dim))
            num_row = random_num
        else:
            ...
            for field_name, field_data in file_data.items():
                insert_data.append(construct_data(field_name, field_type_map[field_name], field_data))
        echo(
            client.json().insert(
                collection_name=name,
...
@compose(
    ...
    option('-vd', '--vector-data-file',
           type=Path(),
           prompt_required=False,
           prompt='Vector data file', help='Vector data file'),
...
@compose(
    ...
    consistency_level(),
    option('-qp', '--query-param', 'query_params', multiple=True, help='Query params, format: [key]:[value]'),
)
@pass_obj
def query(cli, name, partition_names, expr, fields,
          limit, consistency_level, query_params):
    client: milvus_connector.Milvus = cli()
    with client:
        if len(fields) == 0:
            echo('Please provide one or more fields, like: -f field1 -f field2')
...
@compose(
    ...
    option('-it', '--index-type',
           type=Choice(INDEX_TYPE_ARRAY, case_sensitive=False),
           default="IVF_FLAT",
           prompt_required=False,
           prompt='Index type', help='Index type'),
    option('-mt', '--metric-type',
           type=Choice(METRIC_TYPE_ARRAY, case_sensitive=False),
           default="L2",
           prompt_required=False,
           prompt='Metric type', help='Metric type'),
    option('-nl', '--nlist', type=int,
           default=128,
           prompt_required=False,
           prompt='Nlist', help='Nlist'),
)
@pass_obj
def create_index(cli, name, index_name, field_name,
                 index_type, metric_type, nlist):
    client: milvus_connector.Milvus = cli()
    with client:
        ...
        echo(client.json().create_index(collection_name=name,
...
def load_progress(cli, name, partition_names):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().get_loading_progress(collection_name=name, partition_names=partition_names))
...
def load_partitions(cli, name, partition_names, num):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().load_partitions(collection_name=name,
                                           partition_names=partition_names,
                                           replica_number=num,
                                           ))
...
def release_partitions(cli, name, partition_names):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().release_partitions(collection_name=name,
                                              partition_names=partition_names,
                                              ))
...
def show_partitions(cli, name, cid):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().show_partitions(collection_name=name,
                                           collection_id=cid,
                                           ))
...
def partition_statistics(cli, name, partition_name):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().get_partition_statistics(
            collection_name=name,
            partition_name=partition_name,
        ))
...
def operate_user_role(cli, username, rolename, add):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().operate_user_role(
            username=username,
            role_name=rolename,
...
@compose(
    ...
    option('-ot', '--object-type',
           type=Choice([
               "collection",
                "global",
                "user",
               ], case_sensitive=False),
           prompt='Object type', help='Object type'),
...
def operate_privilege(cli, database, rolename, object_name, object_type, grant, privilege):
    client: milvus_connector.Milvus = cli()
    with client:
        echo(client.json().operate_privilege(
            db_name=database,
            role_name=rolename,
...
@compose(
    ...
    database_name(),
    rolename(),
    option('-on', '--object-name',
           prompt_required=False,
           prompt='Object name', help='Object name'),
    option('-ot', '--object-type',
           prompt_required=False,
           type=Choice([
               "collection",
